...
int main()
{
    ...
    //Receive input from the calculator's form
    app().registerHandler(
        "/postinput",
        [](const HttpRequestPtr &req,
           std::function<void(const HttpResponsePtr &)> &&callback) {
        ...
        bool foundOperator;
        int operatorPosition = -1; //initialize the variable with a negative value indicating no operator has been found
        ...
        for (const auto &param : formParams) {
            ...
            if (key == "modulus") {
                ...
                if (currentValuePair.second.empty()) {
                    currentValue = currentValuePair.first;
                    std::cout << "Result: " << currentValuePair.first << std::endl;
                } else { // if there is an error in the calculation, display that in the html input box
                    std::cerr << currentValuePair.second << std::endl;
                    input.clear();
                ...
                if (lastElement == "Invalid expression") { // if "Invalid expression" is being shown and another button is clicked, first clear the input box
                    input.clear();
                }
                ...
                if ((key == "divide" || key == "multiply" || key == "add") && input.size() < 1 ){/*Prohibit an operator
					...
					input.push_back(value);
				} else if (req->getSession()->get<bool>("isOperator") == true ){ /*If an operator is clicked while the input box is displaying an answer
